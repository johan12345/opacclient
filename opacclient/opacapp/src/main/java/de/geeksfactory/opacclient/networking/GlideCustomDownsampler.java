 ...
 */
public abstract class GlideCustomDownsampler extends Downsampler {
    private static final String TAG = "Downsampler";
    private static final String BAD = "f39eeebf13f07bbe192df40a8a0214f9";
     ...
     * smallest edge of the image will be between 1x and 2x the requested size. The larger edge has
     * no maximum size.
     */
    public static final GlideCustomDownsampler AT_LEAST = new GlideCustomDownsampler() {
        @Override
        ...
        @Override
        public String getId() {
            return "AT_LEAST.de.geeksfactory.opacclient.networking";
        }
    };

    /**
     * Load and scale the image uniformly (maintaining the image's aspect ratio) so that largest
     * edge of the image will be between 1/2x and 1x of the requested size. The smaller edge has no
     * minimum size.
     */
    public static final GlideCustomDownsampler AT_MOST = new GlideCustomDownsampler() {
        @Override
        protected int getSampleSize(int inWidth, int inHeight, int outWidth, int outHeight) {
        ...
        @Override
        public String getId() {
            return "AT_MOST.de.geeksfactory.opacclient.networking";
        }
    };
     ...
     */
    public static final GlideCustomDownsampler NONE = new GlideCustomDownsampler() {
        @Override
        protected int getSampleSize(int inWidth, int inHeight, int outWidth, int outHeight) {
            return 0;
        }
        ...
        @Override
        public String getId() {
            return "NONE.de.geeksfactory.opacclient.networking";
        }
    };
     ...
     * those of the image for the given InputStream is available, the operation is much less
     ...
     * @return A new bitmap containing the image from the given InputStream, or recycle if recycle
     * is not null.
     */
    ...
    public Bitmap decode(InputStream is, BitmapPool pool, int outWidth, int outHeight,
        ...
        // TODO(#126): when the framework no longer returns partially decoded Bitmaps or provides
        // a way to determine
        ...
        try {
            ...
            try {
                ...
                DigestInputStream dis = new DigestInputStream(invalidatingStream, md);
                while (dis.read(bytesForStream) != -1) ;
                byte[] digest = md.digest();
                String hex = byteArray2Hex(digest);
                if (hex.equals(BAD)) return null;
                System.out.println(hex);
            } catch (NoSuchAlgorithmException | IOException e) {
                e.printStackTrace();
            }
            try {
                invalidatingStream.reset();
            } catch (IOException e) {
                e.printStackTrace();
            }
            ...
            // BitmapFactory swallows exceptions during decodes and in some cases when inBitmap
            // is non null, may catch
            // and log a stack trace but still return a non null bitmap. To avoid displaying
            // partially decoded bitmaps,
            // we catch exceptions reading from the stream in our ExceptionCatchingInputStream
            // and throw them here.
    ...
    private int getRoundedSampleSize(int degreesToRotate, int inWidth, int inHeight, int outWidth,
        ...
        if (degreesToRotate == 90 || degreesToRotate == 270) {
            // If we're rotating the image +-90 degrees, we need to downsample accordingly so the
            // image width is
            // decreased to near our target's height and the image height is decreased to near
            // our target width.
        ...
        // BitmapFactory only accepts powers of 2, so it will round down to the nearest power of
        // two that is less than
        // or equal to the sample size we provide. Because we need to estimate the final image
        // width and height to
        // re-use Bitmaps, we mirror BitmapFactory's calculation here. For bug, see issue #224.
        // For algorithm see
        ...
        // Although functionally equivalent to 0 for BitmapFactory, 1 is a safer default for our
        // code than 0.
    ...
    private static boolean shouldUsePool(InputStream is) {
        ...
        try {
            ...
            // look at : https://groups.google
            // .com/forum/#!msg/android-developers/Mp0MFVFi1Fo/e8ZQ9FGdWdEJ
    ...
    private static Bitmap.Config getConfig(InputStream is, DecodeFormat format) {
        ...
        // We probably only need 25, but this is safer (particularly since the buffer size is >
        // 1024).
        is.mark(1024);
        try {
            ...
            if (Log.isLoggable(TAG, Log.WARN)) {
                Log.w(TAG,
                        "Cannot determine whether the image has alpha or not from header for " +
                                "format " +
                                format, e);
            }
        } finally {
    ...
    /**
     * Determine the amount of downsampling to use for a load given the dimensions of the image to
     * be downsampled and the dimensions of the view/target the image will be displayed in.
     *
     ...
     * @return An integer to pass in to {@link BitmapFactory#decodeStream(InputStream,
    ...
    private static Bitmap decodeStream(MarkEnforcingInputStream is,
        ...
        if (options.inJustDecodeBounds) {
            // This is large, but jpeg headers are not size bounded so we need something large
            // enough to minimize
            ...
            // Once we've read the image header, we no longer need to allow the buffer to expand
            // in size. To avoid
